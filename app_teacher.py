...
if df is not None:
    # 使用 Session State 鎖定頁面，換頁必重整
    if 'page_idx' not in st.session_state:
        st.session_state.page_idx = 0

    # --- 頂部控制列 ---
    c_sel, c_sync = st.columns([3, 1])
    
    with c_sel:
        page_labels = [f"第 {i+1} 頁" for i in range(len(df))]
        selected_label = st.selectbox("🎯 選擇教學頁碼", page_labels, index=st.session_state.page_idx)
    ...
    with c_sync:
        st.write("###") # 補位
        if st.button("🔄 同步錄音 (清除快取)", use_container_width=True):
            st.rerun()
    ...
    try:
        ...
        <style>
            ...
            .header-bar {{ display: flex; align-items: center; justify-content: space-between; padding: 15px 25px; border-bottom: 3px solid #1e40af; background: #f8fafc; }}
            .title {{ color: #1e40af; font-size: 32px; font-weight: 900; }}
            .play-btn {{ background: #1e40af; color: white; padding: 15px 35px; border-radius: 50px; border: none; font-size: 22px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 12px rgba(30,64,175,0.3); }}
            .pdf-view {{ width: 100%; }}
            ...
            input[type=range] {{ flex: 1; accent-color: #1e40af; height: 15px; cursor: pointer; }}
            .time-box {{ font-size: 18px; color: #1e293b; min-width: 110px; text-align: right; font-family: monospace; font-weight: bold; }}
            .subtitle-stage {{ width: 100%; min-height: 220px; display: flex; flex-direction: column; padding: 30px; box-sizing: border-box; }}
            .bubble {{ max-width: 80%; padding: 25px; border-radius: 25px; box-shadow: 0 10px 30px rgba(0,0,0,0.08); font-size: 32px; line-height: 1.5; opacity: 0; transition: 0.2s ease; }}
            .yj {{ align-self: flex-start; background: #e0f2fe; color: #0369a1; border: 2px solid #bae6fd; border-bottom-left-radius: 5px; }}
            .xz {{ align-self: flex-end; background: #fff1f2; color: #be123c; border: 2px solid #fecdd3; border-bottom-right-radius: 5px; }}
            .name {{ font-size: 16px; font-weight: bold; margin-bottom: 10px; opacity: 0.8; }}
        </style>
        </head>
        <body>
            <div class="header-bar">
                <div class="title">🚀 {selected_label} 教學演示</div>
