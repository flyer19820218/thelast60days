...
if df is not None:
    ...
    if len(pdf_b64) > 500:
        ...
        try:
            ...
            # --- 🔥 按鈕組件 (獨立於 Python f-string 之外，避免大括號衝突) 🔥 ---
            # 這裡我們用一個簡單的方法：將 JavaScript 代碼封裝成不包含大括號或正確轉義的版本
            btn_html = f"""
            <div id="playBtn" style="background:linear-gradient(135deg,#2b58db,#1d4ed8);color:white;padding:8px 15px;border-radius:50px;display:flex;align-items:center;justify-content:center;gap:8px;font-family:sans-serif;font-weight:bold;font-size:15px;cursor:pointer;box-shadow:0 4px 10px rgba(29,78,216,0.3);user-select:none;white-space:nowrap;">
            ...
            <style>
                ...
                .seek-bar-container {{ width: 100%; padding: 10px 0; display: flex; align-items: center; gap: 10px; }}
                ...
                .bubble-box {{ background: #fff9f0; border: 2px dashed #fed7aa; border-radius: 15px; padding: 15px; min-height: 60px; display: flex; flex-direction: column; justify-content: center; align-items: center; margin-top: 5px; }}
